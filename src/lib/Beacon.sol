...
library Beacon {
    ...
    function findBlockRoot(uint256 genesisBlockTimestamp, uint64 slot) public view returns (bytes32 blockRoot) {
        uint256 currentBlockTimestamp = genesisBlockTimestamp + ((slot + 1) * BLOCK_SPEED);

        uint256 earliestBlockTimestamp = block.timestamp - (BEACON_ROOTS_HISTORY_BUFFER_LENGTH * BLOCK_SPEED);
        if (currentBlockTimestamp < earliestBlockTimestamp) {
            revert TimestampOutOfRange();
        }

        while (currentBlockTimestamp <= block.timestamp) {
            (bool success, bytes memory result) = BEACON_ROOTS_ADDRESS.staticcall(abi.encode(currentBlockTimestamp));
            if (success && result.length > 0) {
                return abi.decode(result, (bytes32));
            }

            unchecked {
                currentBlockTimestamp += BLOCK_SPEED;
            }
        }
    ...
    function epochTimestamp(uint256 genesisBlockTimestamp, uint64 epoch) external pure returns (uint256 timestamp) {
        timestamp = genesisBlockTimestamp + epoch * BLOCKS_PER_EPOCH * BLOCK_SPEED;
    }
}
